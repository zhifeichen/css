...
int css_get_db_config(css_db_config* myDbConfig)
{
	...
	css_get_env(SECTION_DB, "host", default_db_config.host,
			&(myDbConfig->host));

	css_get_env(SECTION_DB, "type", db_enum2char(default_db_config.dbType), &s);
	myDbConfig->dbType = db_char2enum(s);
	FREE(s);

	css_get_env(SECTION_DB, "user", default_db_config.user,
			&(myDbConfig->user));

	css_get_env(SECTION_DB, "passwd", default_db_config.passwd,
			&(myDbConfig->passwd));

	css_get_env(SECTION_DB, "name", default_db_config.dbName,
			&(myDbConfig->dbName));

	css_get_env(SECTION_DB, "port", ITOA(default_db_config.port, t, 10), &s);
	myDbConfig->port = atoi(s);
	FREE(s);
...
int css_get_disk_config(css_disk_config* myDiskConfig)
{
	...
	css_get_env(SECTION_DISK, "max_per",
			ITOA(default_disk_config.max_per, t, 10), &s);
	myDiskConfig->max_per = atoi(s);
	FREE(s);
	memset(t, 0, 32);

	css_get_env(SECTION_DISK, "volume_path", default_disk_config.disk_paths,
			&(myDiskConfig->disk_paths));

	css_get_env(SECTION_DISK, "clear_file_day",
			ITOA(default_disk_config.clear_file_day, t, 10), &s);
	myDiskConfig->clear_file_day = atoi(s);
	FREE(s);
